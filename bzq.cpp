...
void BFS()
{
	...
	while(!q.empty())
	{
		...
		int x=tmp.x,y=tmp.y;
		if(dt[x][y].sz<bz[x][y]) continue;
		for(int i=1;i<=4;i++)
		{
			int xx=x+dx[i],yy=y+dy[i];
			if(xx>8||yy>8||xx<1||yy<1) continue;
			if(dt[x][y].sz<bz[x][y]) continue;
			dt[xx][yy].player=dt[x][y].player;
			dt[xx][yy].sz++;
			if(dt[xx][yy].sz>=bz[xx][yy]) q.push(dl{tmp.c+1,xx,yy});
		}
		if(dt[x][y].sz>=bz[x][y])
			dt[x][y].sz-=bz[x][y];
		if(dt[x][y].sz==0)
...
//	scanf("%d%d",&x,&y);
	...
	if(x>8||y>8||x<1||y<1){printf("此位置已出界\n");S;goto hg;}
	if(dt[x][y].player!=t&&dt[x][y].player!=0){printf("位置已被他人占领\n");S;goto hg;}
	dt[x][y].sz+=1;
	dt[x][y].player=t;
	if(dt[x][y].sz<bz[x][y]) return;
	q.push(dl{1,x,y});
	cls;
	BFS();
